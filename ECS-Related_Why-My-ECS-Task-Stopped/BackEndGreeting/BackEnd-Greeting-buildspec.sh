...
LOCAL_PRIVATE_REPOSITORY_NAME=backendgreeting
DOCKER_PRIVATE_REPOSITORY_NAME=dushyant8858/backendgreeting
COMMIT_HASH=$(git rev-parse --short=6 HEAD)


echo ******** Building form ARCH=arm64v8 ********
# docker build -t $LOCAL_PRIVATE_REPOSITORY_NAME:latest-arm64v8 --build-arg ARCH=arm64v8 --build-arg server_port=8080 --no-cache .
docker build -t $LOCAL_PRIVATE_REPOSITORY_NAME:latest-arm64v8 --build-arg ARCH=arm64v8 --build-arg server_port=8080 .
...
# docker build -t $LOCAL_PRIVATE_REPOSITORY_NAME:latest-amd64 --build-arg ARCH=amd64 --build-arg server_port=8080 --no-cache .
docker build -t $LOCAL_PRIVATE_REPOSITORY_NAME:latest-amd64 --build-arg ARCH=amd64 --build-arg server_port=8080 .
